...
# get the coil index
	n=0
	while read line; do
		bart slice 3 $line dataT_final tmpd-${n}
		n=$(($n+1))
	done < $input_coil_index

	bart join 3 $(seq -f "tmpd-%g" 0 $(($n-1))) dataT_final_coil_sel
...
bart transpose 5 10 traj traj2
bart scale 0.5 traj2 $out_traj
...
bart index 5 $nf1 tmp1
bart scale $(($nspokes_per_frame * $TR * $slices)) tmp1 tmp2
bart ones 6 1 1 1 1 1 $nf1 tmp3
bart saxpy $((($nspokes_per_frame * $slices / 2) * $TR)) tmp3 tmp2 tmp4
bart scale 0.000001 tmp4 $out_TI
