...
if __name__ == "__main__":
	...
	out_file = "bland_T1_brain_ref.pdf"
	fig = plt.figure()
	ax = fig.add_subplot(1, 1, 1)
	bland_altman_plot_1(ref[:,0]*1000, vol97_scan1[:,0]*1000, out_file, -100, 100, True) # "*1000" [s] -> [ms]
	...
	out_file = "bland_R2s_brain_ref.pdf"
	fig = plt.figure()
	ax = fig.add_subplot(1, 1, 1)
	bland_altman_plot_1(ref[:,1], vol97_scan1[:,1], out_file, -6, 6)
	...
	# Water T1: Scan 2 vs. Scan 1, VOL97

	out_file = "bland_T1_brain_scan.pdf"
	fig = plt.figure()
	ax = fig.add_subplot(1, 1, 1)
	bland_altman_plot_1(np.append(vol97_scan2[:,0]*1000,vol96_scan2[:,0]*1000), np.append(vol97_scan1[:,0]*1000, vol96_scan1[:,0]*1000), out_file, -100, 100, True) # "*1000" [s] -> [ms]
	# ax.set_xticks(np.arange(600, 1600, 400))
	ax.grid()
	plt.xlabel("$T_{1}$ Average / ms")
	plt.ylabel("$T_{1}$ Difference / ms \n Scan #2 - Scan #1")
	...
	# R2s: Scan 1 vs. Scan 2, VOL97

	out_file = "bland_R2s_brain_scan.pdf"
	fig = plt.figure()
	ax = fig.add_subplot(1, 1, 1)
	bland_altman_plot_1(np.append(vol97_scan2[:,1],vol96_scan2[:,1]), np.append(vol97_scan1[:,1], vol96_scan1[:,1]), out_file, -6, 6)
	# ax.set_xticks(np.arange(14, 23, 2))
	ax.grid()
	plt.xlabel("$R_{2}^{*}$ Average / s$^{-1}$")
	plt.ylabel("$R_{2}^{*}$ Difference / s$^{-1}$ \n Scan #2 - Scan #1")
