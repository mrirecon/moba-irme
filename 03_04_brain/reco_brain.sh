...
then
        ...
        --normalize_scaling --scale_data 500 --scale_psf 500 --other pinit=1:$scaling_fat:$scaling_B0:1:1:1:1:1,pscale=1:$scaling_fat:$scaling_B0:1:1:1:1:1,echo=tmp-TE_moba-3e -B0.0 -b 22:8 --other pscale=0.5:$scaling_r2s:$scaling_B0:1. \
        -t tmp-TRAJ_moba-3e tmp-kdat_moba-3e TI-tmp reco-wf-3e sens-wf-3e

        # Prepare initialization file
	## Compensate for internal scaling by "1/$scaling" and 1/1000 because map used ms internally
...
then
	...
	bart moba -i$newton -d4 -g -D -m$recon_type -R3 --img_dims $((dimx/2)):$((dimx/2)):1 -o$overgrid -k --kfilter-2 -C250 -j$lambda \
	--normalize_scaling --scale_data 500 --scale_psf 500 --other pinit=1:1:1:1:1:1:$scaling_r2s:$scaling_B0,pscale=1:1:1:1:1:1:$scaling_r2s:$scaling_B0,echo=$TE \
	-B0. -I M_init -b $fB0_a:$fB0_b \
	...
	bart moba -i$newton -d4 -g -D -m$recon_type -R3 --img_dims $((dimx/2)):$((dimx/2)):1 -o$overgrid -k --kfilter-2 -C250 -j$lambda \
	--normalize_scaling --scale_data 500 --scale_psf 500 --other pinit=1:1:1:$scaling_r2s:$scaling_B0:1:1:1,pscale=1:1:1:$scaling_r2s:$scaling_B0:1:1:1,echo=$TE \
	-B0. -I M_init2 -b $fB0_a:$fB0_b \
	-t TRAJ_moba kdat_moba TI_moba $reco $sens
fi
